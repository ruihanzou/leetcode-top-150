...
class MinimumSizeSubarraySum209 {
     ...
     *    具备单调性，可以用滑动窗口。
     *
     * 2. right 从 0 到 n-1 扫描，每到一个位置就把 nums[right] 加到 windowSum。
     *
     * 3. 只要 windowSum >= target，就更新最小长度，然后 left 右移缩小窗口。
     *    left 右移会减小 windowSum，直到 windowSum < target 为止。
     *
     ...
     *    用二分查找找 prefix 中第一个 > prefix[r+1] - target 的位置 pos，
     *    那么 pos - 1 就是满足条件的最大 l。
     *
     * 4. 利用 Arrays.binarySearch 或手写二分实现 upper_bound。
     *
     * 【举例】target = 7, nums = [2,3,1,2,4,3]
     *   prefix = [0, 2, 5, 6, 8, 12, 15]
     *   r=3 (prefix[4]=8): 找 prefix[l] <= 8-7=1
     *       upper_bound(1) 指向 prefix[1]=2, pos=1, l_max=0, 长度=4
     *   r=4 (prefix[5]=12): 找 prefix[l] <= 12-7=5
     *       upper_bound(5) 指向 prefix[3]=6, pos=3, l_max=2, 长度=3
     *   r=5 (prefix[6]=15): 找 prefix[l] <= 15-7=8
     *       upper_bound(8) 指向 prefix[5]=12, pos=5, l_max=4, 长度=2
     *   答案 = 2
     *
     * 【时间复杂度】O(n log n) —— n 次二分查找
     * 【空间复杂度】O(n) —— 前缀和数组
     */
    public int minSubArrayLenPrefixBinary(int target, int[] nums) {
        int n = nums.length;
        ...
        int minLen = n + 1;
        for (int r = 0; r < n; r++) {
            long needed = prefix[r + 1] - target;
            int pos = upperBound(prefix, needed, 0, r + 1);
            if (pos > 0) {
                minLen = Math.min(minLen, r - (pos - 1) + 1);
            }
        }
