...
class Solution:
    ...
    【核心思路】
    维护一个窗口 [left, right]，用哈希集合 char_set 记录窗口内的所有字符。
    ...
    【核心思路】
    用哈希表 char_index 记录每个字符最近一次出现的位置。
    当 right 遇到重复字符时，left 直接跳到 char_index[s[right]] + 1，
    ...
    2. 用哈希表 char_index[c] = j 表示字符 c 上一次出现在位置 j。
       当 s[right] 在表中且 char_index[s[right]] >= left 时，
       说明窗口内有重复，left 直接跳到 char_index[s[right]] + 1。

    3. 注意 left 只能往右跳不能往左，所以取 max(left, char_index[...] + 1)。

    4. 每次更新 char_index[s[right]] = right。

    【举例】s = "abba"
      right=0 'a': char_index={a:0}, left=0, len=1
      right=1 'b': char_index={a:0,b:1}, left=0, len=2
      right=2 'b': 'b'上次在1, left=max(0,1+1)=2, char_index={a:0,b:2}, len=1
      right=3 'a': 'a'上次在0, 但0<left=2, 不跳, left=2, char_index={a:3,b:2}, len=2
      答案 = 2
