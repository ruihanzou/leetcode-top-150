...
class Solution:
    ...
    【空间复杂度】O(1)，不计输出链表的空间
    """
    def addTwoNumbers_iterative(self, l1: Optional[ListNode], l2: Optional[ListNode]) -> Optional[ListNode]:
        dummy = ListNode(0)
        curr = dummy
        carry = 0
    ...
    【思考过程】
    1. 递归的终止条件：l1 is None and l2 is None and carry == 0。

    2. 每层递归：
       - 取出 l1 和 l2 当前位的值（如果为 None 则取 0）
       - 计算 total = val1 + val2 + carry
       - 创建当前节点，值为 total % 10
       - 递归处理 next，传入新的 carry = total // 10
    ...
    【举例】l1 = [2,4,3], l2 = [5,6,4]
      ...
      递归第3层：3+4+1=8, 创建节点8, carry=0 → 递归(None, None, 0)
      递归第4层：终止条件满足，返回 None
      回溯拼接：7 → 0 → 8
