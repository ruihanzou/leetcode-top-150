...
class Solution:
    ...
    - 偶数层：从右端 append（正常从左到右顺序）
    - 奇数层：从左端 appendleft（实现从右到左顺序）
    这样避免了事后翻转。
    ...
    2. 思路：遍历节点的顺序不变（始终从左到右出队），
       但收集节点值时改变插入方向。
       - 从左到右层：append 到右端 → 顺序 [a, b, c]
       - 从右到左层：appendleft 到左端 → 顺序 [c, b, a]

    3. 注意：子节点入队顺序始终是先左后右，不改变。
       只改变"值插入 level 的方向"。

    【举例】root = [3, 9, 20, null, null, 15, 7]
      第0层(→): 出队3, append → level=[3]
      第1层(←): 出队9, appendleft → [9];
                出队20, appendleft → [20, 9]
      第2层(→): 出队15, append → [15];
                出队7, append → [15, 7]
      结果: [[3], [20,9], [15,7]]
