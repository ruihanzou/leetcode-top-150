...
class LengthOfLastWord058 {
     ...
     *
     * 【思考过程】
     * 1. split("\\s+") 可以按一个或多个空格分割，自动处理连续空格。
     *
     * 2. 注意 Java 的 split 如果字符串开头有空格，第一个元素会是空字符串。
     *    但使用 trim() 先去除首尾空格，或者用 "\\s+" 正则，可以避免问题。
     *
     * 3. 分割后直接取最后一个元素的长度即可。
     *
     * 【举例】s = "Hello World"
    ...
    /**
     * ==================== 解法三：trim + lastIndexOf ====================
     *
     * 【核心思路】
     * 先 trim 去除首尾空格，然后找最后一个空格的位置，
     * 最后一个单词的长度 = 总长度 - 最后一个空格位置 - 1。
     *
     * 【思考过程】
     * 1. trim 后字符串不会以空格结尾，所以最后一个空格之后的所有字符
     *    就是最后一个单词。
     *
     * 2. lastIndexOf(' ') 返回最后一个空格的下标。
     *    如果没有空格（整个字符串就是一个单词），返回 -1，
     *    此时长度 = len - (-1) - 1 = len，正确。
     *
     * 【举例】s = "Hello World"
     *   trim 后仍是 "Hello World"，长度 11
     *   lastIndexOf(' ') = 5
     *   长度 = 11 - 5 - 1 = 5
     *
     * 【时间复杂度】O(n)
     * 【空间复杂度】O(n) trim 创建新字符串
     */
