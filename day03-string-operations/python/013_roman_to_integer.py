...
class Solution:
    ...
    【空间复杂度】O(1)
    """
    def roman_to_int_left_to_right(self, s: str) -> int:
    ...
    【空间复杂度】O(n) 替换后的新字符串
    """
    def roman_to_int_replace(self, s: str) -> int:
        replacements = [
            ('CM', 'f'), ('CD', 'e'), ('XC', 'd'),
            ('XL', 'c'), ('IX', 'b'), ('IV', 'a')
        ]
        ...
        return sum(mapping[ch] for ch in s)
    ...
    【空间复杂度】O(1)
    """
    def roman_to_int_right_to_left(self, s: str) -> int:
        mapping = {'I': 1, 'V': 5, 'X': 10, 'L': 50,
                   'C': 100, 'D': 500, 'M': 1000}
        result = 0
        prev = 0
