...
class Solution:
    ...
    有点和分糖果有点类似，都是需要考虑左右两边的情况。

    要理解双指针解法的**思考源头**，核心是从「前后缀数组解法」的痛点出发，一步步推导优化——它不是凭空想出来的，而是对“空间优化”的自然探索。
   
        【思考递进】
        我用「思考递进」的方式，带你还原从“前后缀”到“双指针”的完整推导过程：

        ### 第一步：先锚定核心公式（所有解法的根基）
        不管是哪种解法，接雨水的核心逻辑永远是：
        > 位置 `i` 的积水量 = min(左边最高柱子高度, 右边最高柱子高度) - 当前柱子高度

        前后缀数组解法就是先把所有位置的「左最高」和「右最高」都算出来存起来，再逐个计算。

        ### 第二步：发现前后缀解法的“冗余”——空间可以省
        前后缀解法需要用两个数组存 `left_max` 和 `right_max`，但我们真的需要**提前存所有位置**的左右最高吗？
        👉 关键反问：计算位置 `i` 的积水量，真的需要知道「整个右边的最大值」吗？

        举个例子：
        假设我们有指针 `left` 停在位置2，`right` 停在位置10（对应示例 `[0,1,0,2,1,0,1,3,2,1,2,1]`）：
        - 此时 `left_max=1`（left左边的最高），`right_max=2`（right右边的最高）；
        - 因为 `left_max < right_max`，所以不管 `right` 左边还有没有更高的柱子（比如位置7的3），`min(left_max, 右边任意更高值)` 永远是 `left_max`；
        - 也就是说：**只要知道「当前左最大 < 当前右最大」，左指针位置的积水量就可以确定了，不用等所有右最大都算完**。

        这是双指针最核心的「顿悟点」——我们不需要全局的左右最大，只需要「动态的、当前的」左右最大，就能确定某一侧指针位置的积水量。

        ### 第三步：推导指针移动规则
        基于上面的发现，我们可以设计两个指针从两端向中间走，同时维护 `left_max`（左指针左侧的最大值）和 `right_max`（右指针右侧的最大值）：
        1. 初始：`left=0`（最左），`right=len(height)-1`（最右），`left_max=right_max=0`；
        2. 比较 `left_max` 和 `right_max`：
        - 如果 `left_max ≤ right_max`：左指针位置的积水量 = `left_max - height[left]`（因为右边的最大值至少是 `right_max`，比 `left_max` 大，min就是 `left_max`），然后 `left++`，并更新 `left_max`；
        - 如果 `left_max > right_max`：右指针位置的积水量 = `right_max - height[right]`（同理，左边的最大值至少是 `left_max`，min就是 `right_max`），然后 `right--`，并更新 `right_max`；
        3. 直到 `left >= right`，所有位置的积水量都算完了。

        ### 第四步：用示例验证逻辑（让思考落地）
        还是以 `height = [0,1,0,2,1,0,1,3,2,1,2,1]` 为例，模拟双指针的思考过程：
        - 初始：left=0, right=11, left_max=0, right_max=1；
        - `left_max(0) ≤ right_max(1)`：计算left=0的积水（0-0=0），left=1，left_max更新为1；
        - `left_max(1) ≤ right_max(1)`：计算left=1的积水（1-1=0），left=2，left_max保持1；
        - `left_max(1) ≤ right_max(1)`：计算left=2的积水（1-0=1），left=3，left_max更新为2；
        - `left_max(2) > right_max(1)`：计算right=11的积水（1-1=0），right=10，right_max更新为2；
        - ...... 以此类推，最终累加出总和6。

        ### 总结：双指针的思考路径
        1. 先掌握「核心公式」（min(左最高, 右最高) - 当前高度）；
        2. 发现前后缀解法的空间冗余：不需要提前存所有左右最高；
        3. 关键观察：「当前左最大 < 当前右最大」→ 左指针位置的积水可确定（反之同理）；
        4. 设计双指针+动态维护左右最大，把空间从O(n)优化到O(1)。

        这个推导过程的核心是「先锚定核心逻辑，再找优化点，最后验证规则」——面试时你按这个思路跟面试官讲，会比只背代码更能体现你的思考能力。
