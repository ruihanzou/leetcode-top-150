...
class Solution:
    ...
    2. 两棵树"镜像"可以递归定义：
       ...
       - 一个 None → 不镜像
       - 值不等 → 不镜像
       - 值相等 → A 的左 与 B 的右 镜像，且 A 的右 与 B 的左 镜像
    ...
    1. 递归的镜像比较可以用队列展开：
       ...
       - 每次取出一对 (a, b)：
         都 None → 继续；一个 None → False；值不等 → False
    ...
    【举例】root = [1,2,2,null,3,null,3]
      queue = [(2, 2)]
      取出 (2,2)：值相等，入队 (2.left, 2.right) 和 (2.right, 2.left)
        → 入队 (None, None) 和 (3, None)... 不对，镜像规则：
        → 入队 (2.left, 2.right) = (None, 3) 和 (2.right, 2.left) = (3, None)
      取出 (None, 3) → 一个为 None → False
