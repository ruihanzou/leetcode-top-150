...
示例 1：nums1 = [1,7,11], nums2 = [2,4,6], k = 3
  输出：[[1,2],[1,4],[1,6]]
  解释：前 3 对为 [1,2],[1,4],[1,6]，它们的和分别为 3,5,7。
...
class Solution:
    ...
    """
    def kSmallestPairs_bfs(self, nums1: List[int], nums2: List[int], k: int) -> List[List[int]]:
    ...
    【空间复杂度】O(min(m, k))
    """
    def kSmallestPairs_column(self, nums1: List[int], nums2: List[int], k: int) -> List[List[int]]:
        if not nums1 or not nums2:
            return []
