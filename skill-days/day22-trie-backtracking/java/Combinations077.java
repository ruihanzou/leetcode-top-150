...
class Combinations077 {
     ...
     *    还需要选 k - path.size() 个数，可选范围是 [i, n]，共 n-i+1 个。
     *    如果 n - i + 1 < k - path.size()，即 i > n - (k - path.size()) + 1，
     *    就不必继续了。
     *
     * 【举例】n = 4, k = 2
     *   start=1:
     ...
     *     选3 → start=4:
     *       选4 → [3,4] ✓
     *     选4 → start=5: 还需1个数但没有了 → 剪枝（实际这里 path=[4] 只有1个，需要再选1个但 start=5 > n=4）
     *
     ...
     *    - 将 combo[j]++，然后 combo[j+1] = combo[j]+1, combo[j+2] = combo[j]+2, ...
     *
    ...
    public List<List<Integer>> combineIterative(int n, int k) {
        ...
        while (true) {
            ...
            for (int val : combo) current.add(val);
            result.add(current);

            int j = k - 1;
            while (j >= 0 && combo[j] == n - k + j + 1) {
                j--;
            }
            if (j < 0) break;
