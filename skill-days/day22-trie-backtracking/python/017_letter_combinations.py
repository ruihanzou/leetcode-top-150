...
class Solution:
    ...
    2. 用回溯框架：
       ...
       - 选择列表：当前数字对应的所有字母。
       - 终止条件：index == len(digits)，路径长度等于数字个数。
    ...
    def _backtrack(self, digits: str, index: int, path: list, result: List[str]):
        if index == len(digits):
            result.append(''.join(path))
            return
        for ch in MAPPING[digits[index]]:
            path.append(ch)
            self._backtrack(digits, index + 1, path, result)
            path.pop()
    ...
    def letterCombinationsBFS(self, digits: str) -> List[str]:
        ...
        for digit in digits:
            ...
            for _ in range(len(queue)):
                existing = queue.popleft()
                for ch in letters:
                    queue.append(existing + ch)
