...
class Solution:
    ...
    3. 用一个计数器 self.count 记录已经访问了多少个节点，
       当 count == k 时记录答案并返回。
    ...
    """
    def kthSmallest_inorder_recursive(self, root: Optional[TreeNode], k: int) -> int:
        self.count = 0
        self.result = 0
    ...
    """
    def kthSmallest_inorder_iterative(self, root: Optional[TreeNode], k: int) -> int:
        stack = []
        curr = root
