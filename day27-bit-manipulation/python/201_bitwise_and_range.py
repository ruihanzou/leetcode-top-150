...
class Solution:
    ...
    2. right >= left，说明 right 比 left 多了一些高位的 1 或者低位不同。
    3. right & (right-1) 会消掉 right 最右边的 1。
       如果消掉的这个 1 在公共前缀之外，那 right 变小但仍然 >= left。
       如果消到公共前缀内了，right 就会 < left 或 = left。
    ...
    【举例】left = 5 (101), right = 7 (111)
      ...
      等等，4 < 5，但答案确实是 4。让我检查...
      实际上 right=4 < left=5 时停止，但 AND 结果是 4。
      因为 5&6&7 = 101 & 110 & 111 = 100 = 4。
      所以这里应该是 while right > left，最终 right=4 就是答案。
