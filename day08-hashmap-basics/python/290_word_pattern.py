...
class Solution:
    ...
    【核心思路】
    将 s 按空格分割成单词列表，建立 pattern 字符 → 单词 和 单词 → pattern 字符
    ...
    【空间复杂度】O(n)，两个映射字典
    """
    def wordPattern_two_map(self, pattern: str, s: str) -> bool:
        words = s.split()
    ...
    【空间复杂度】O(n)
    """
    def wordPattern_encode(self, pattern: str, s: str) -> bool:
        words = s.split()
        if len(pattern) != len(words):
            return False
        ...
        for i, (c, w) in enumerate(zip(pattern, words)):
            if char_index.get(c, -1) != word_index.get(w, -1):
                return False
            char_index[c] = i
            word_index[w] = i
        return True
