...
class RansomNote383 {
     ...
     *
     * 【核心思路】
     * 用 HashMap 统计 magazine 中每个字符的出现次数，
     ...
     *   全部消耗完毕，返回 true
     *
     * 【时间复杂度】O(m + n)，m = magazine.length, n = ransomNote.length
     ...
     *
     * 【核心思路】
     * 用长度为 26 的 int 数组代替 HashMap，下标 c - 'a' 对应字符 c 的频次。
     * 先加 magazine 的频次，再减 ransomNote 的频次，出现负数即不可构成。
     *
     * 【思考过程】
     * 1. 题目说明只包含小写英文字母，字符范围固定为 26 个，
     *    用数组比 HashMap 更快（无哈希碰撞、无装箱拆箱开销）。
     *
     * 2. 数组下标天然就是字符到整数的映射，arr[c - 'a'] 即字符 c 的计数。
     *
     * 【举例】ransomNote = "aa", magazine = "aab"
